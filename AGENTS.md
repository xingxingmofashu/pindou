...
**`packages/shared/src/palettes.json` is the single source of palette data** — a
committed static catalog of 4 brands (mard/perler/artkal/hama) with their full
color sets (uuid ids, codes, hexes, series, sort_order), bundled into both the
web and desktop apps. It was snapshotted from the original Neon DB seed
(migrations 0006–0008, 0011); the `brands`/`colors` DB tables were **removed in
migration 0014** (FK dropped first, then the tables). Palette changes are made
by editing the JSON and rebuilding — no DB involved. The uuid ids are stable,
so existing `patterns.fk_brand_id` values remain valid.

```
packages/shared/src/palettes.json     The catalog (Palette[] — brands with nested colors, ISO date strings)
packages/shared/src/palettes.ts       PALETTES export — loads the JSON, converts ISO dates → Date
packages/shared/src/types.ts          Brand/Color/Palette types (data source: the JSON, not a DB table)
apps/web/src/lib/server/palettes.ts   getPaletteByCode / getPaletteById / getBrandPalette / getAllPalettes — pure in-memory lookups over PALETTES
apps/web/src/app/api/brands/route.ts  GET — all brands with their colors nested (the client catalog)
apps/web/src/app/api/brands/[id]/route.ts GET — one brand by uuid id with colors nested (no generateStaticParams — runtime lookup)
apps/web/src/db/schema.ts             patterns table only (fk_brand_id is a plain uuid, no FK constraint)
apps/web/src/hooks/use-palette.ts     Editor store (Zustand) — active palette (pushed in by ColorPalette, no fetch of its own)
```

- `code`/`name` are uppercase (e.g. `"A1"`); `series` is nullable — use `c.series ?? "?"`
- Two representations coexist: the editor's **in-memory sparse map holds 1‑based indices** into `palette.colors` (order-dependent), while the **wire/stored grid is code‑based** (`string[][]`, `""` = empty — migration 0012, PR "store pattern grids as colour codes"). Colors are served in `sortOrder` (file order) because the in-memory index and the editor's ordering depend on it. Reordering color entries no longer corrupts *published* patterns (codes are stable), but it does shift in-memory index semantics — `serializeGrid`/`deserializeGrid` take the palette for exactly this reason, and any brand-palette replace must rebuild the code→index binding in code that holds sparse state.
- `/api/brands` is CDN/browser-cacheable: the catalog only changes on a rebuild (committed JSON), so the route sends `Cache-Control: public, max-age=3600, s-maxage=3600, stale-while-revalidate=86400`. Palette changes propagate only after the next deploy + cache expiry.
- Wire contract between client and server is the brand **code** (a plain string matching `code`, e.g. `"mard"`); the server maps code↔brand uuid (`patterns.fk_brand_id`) internally. `patterns.fk_brand_id` is a **plain uuid column** — the FK to `brands.id` was dropped in migration 0014, so validity is enforced in the app layer (`getPaletteByCode` on publish; the uuid is preserved on edit).
- `usePalette()` (in `use-palette.ts`) reads a module-level Zustand store and returns `{ palette, setActivePalette }`; it holds only the active palette (a `Palette` — a brand entry with colors nested) and makes no network requests. ColorPalette fetches `/api/brands` (the catalog) for its switcher, builds the chosen brand's palette, and pushes it into the store. The editor canvas (`EditablePaletteBridge`), the export dialog, and the import dialog read the shared palette because the user-controlled EditorPage cannot wire it as a prop (the export dialog must use the same palette instance the canvas draws with — a freshly fetched brand could serve a cached colour order and shift every bead). Consumers that need a *specific* brand (pattern detail/edit pages) fetch it directly in their server page code via `getBrandPalette` instead of touching the store. Read-only views pin a `palette` prop and bypass the store entirely. SSR snapshots are null, so hydration stays consistent.
...
src/lib/server/palettes.ts         getPaletteByCode(code)/getPaletteById/getAllPalettes — in-memory lookups over the bundled palettes.json (no DB)
src/lib/server/patterns.ts         getPatternsPage/getPattern — `unstable_cache`(30s, tags `patterns`/`pattern`) data access shared by routes + SSR pages; Dates normalized (cache round-trips JSON); brandCode resolved via the palette catalog, not a join
...
- **Tables**: `patterns` only (id uuid PK defaultRandom, title, description, author_name, grid_key, fk_brand_id uuid — **no FK constraint**, fk_user_id → auth.users, bead_stats, thumb_url, timestamps). The `brands`/`colors` tables were removed in migration 0014; brand/color data lives exclusively in `packages/shared/src/palettes.json`. `patterns` rows share the same audit shape as before: uuid `id` (default `gen_random_uuid()`) and `created_at`/`updated_at` (`timestamp with time zone`, default `now()`) — the DB generates them, so routes never set them. `patterns.fk_brand_id` holds a brand uuid from the palette catalog (mard/perler/artkal/hama), resolved to a code in the app layer.
...
- Image→grid conversion is **client-side** (no `/api/transform`): the import worker filters the active palette by `excludedCodes` before building its OKLab samples (all colours excluded → error) and produces a code grid. Publish posts `brandCode` to `/api/patterns`. Both publish and edit resolve the palette from the bundled catalog via `getPaletteByCode`/`getPaletteById` (`lib/server/palettes.ts`) and store the brand uuid in `patterns.fk_brand_id`; the GET paths resolve that uuid back to `brandCode` via the same catalog (no join).
- Migration order matters: `brands`/`colors` existed before `patterns.fk_brand_id` (uuid FK, migration 0002 backfills the old text `brand_id` codes to uuids). Palette data was seeded by the idempotent data migrations 0006–0008/0011, then **removed in migration 0014** (FK dropped first, then both tables) — there is no `db:seed` script; `db:migrate` initializes schema. When changing the schema, run `db:generate` → `db:migrate`.
