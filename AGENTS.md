...
- **Thumbnails (Cloudflare R2)**: on publish, the `Thumbnail` instance (in `lib/image/thumbnail.ts`) calls `generate` to render the grid to a PNG Buffer, then `upload` to store it at `thumbnails/{patternId}.png` via its own `new R2()` instance (class in `lib/r2.ts`) and returns its public URL, which is saved in `patterns.thumb_url`. R2 is a **hard dependency of publishing** — an upload failure fails the publish (503, nothing is written to the DB); there is no base64 fallback. Required env vars: `R2_ENDPOINT_URL`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET_NAME`, `NEXT_R2_PUBLIC_URL` (copy from `.env.example`).
